...
pub type Error = BasicError<u64, std::num::ParseIntError>;
pub type Result = std::result::Result<u64, Error>;

fn as_u32(n: u64) -> std::result::Result<u32, Error> {
    if n > (u32::MAX as u64) {
...
impl Calcable for u64 {
    ...
    fn parse_binary(s: &str) -> Result {
        u64::from_str_radix(&clean_input(s, "0b"), 2).map_err(Error::Parse)
    }

    fn parse_octal(s: &str) -> Result {
        u64::from_str_radix(&clean_input(s, "0o"), 8).map_err(Error::Parse)
    }
    ...
    fn parse_hex(s: &str) -> Result {
        u64::from_str_radix(&clean_input(s, "0x"), 16).map_err(Error::Parse)
    }
